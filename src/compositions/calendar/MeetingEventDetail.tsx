...
import { MetaGrid, AttendeesSection, MEETING_ICONS, MEETING_TYPE_LABELS, VISIBILITY_LABEL } from "./calendar-event-parts";
...
export function MeetingEventDetail({
  event, userNames, currentUserId, organizationId, onRsvpChange,
    ...
    <>
      ...
      {info.description && (
        <div className="rounded-lg bg-muted/50 p-4">
          <p className="text-caption font-medium text-muted-foreground mb-2">Description</p>
      ...
      <AttendeesSection
        eventId={event.id}
        attendees={(info.attendees ?? []).map((a) => ({ userId: a.userId, rsvpStatus: a.rsvpStatus }))}
        userNames={userNames}
        currentUserId={currentUserId}
        guestEmailCount={info.guestEmailCount}
      />
      {organizationId && (
        <><Separator /><AgendaNotesSection organizationId={organizationId} meetingId={event.id} userNames={userNames} /></>
      )}
    </>
  );
}
