...
interface QuickCreateMeetingFormProps {
  ...
  onSave: (data: { title: string; meetingType: string; attendeeUserIds: string[]; guestEmails: string[] }) => void;
  onMoreOptions: (data: { title: string; meetingType: string; attendeeUserIds: string[]; guestEmails: string[] }) => void;
  saving?: boolean;
}

export function QuickCreateMeetingForm({
  defaultAttendeeIds, members, onSave, onMoreOptions, saving,
}: QuickCreateMeetingFormProps) {
  const [title, setTitle] = useState("");
  const [meetingType, setMeetingType] = useState("video");
  const [selectedMemberIds, setSelectedMemberIds] = useState<Set<string>>(() => new Set(defaultAttendeeIds));
  const [guestEmail, setGuestEmail] = useState("");
  const [guestEmails, setGuestEmails] = useState<string[]>([]);

  const formData = useCallback(() => ({
    title: title.trim(), meetingType, attendeeUserIds: Array.from(selectedMemberIds), guestEmails,
  ...
  const handleAddGuest = useCallback(() => {
    ...
    if (email && email.includes("@") && !guestEmails.includes(email)) {
      setGuestEmails((prev) => [...prev, email]); setGuestEmail("");
    }
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-col gap-1">
        <Label htmlFor="meeting-title" className="text-caption">Title</Label>
        <Input id="meeting-title" placeholder="Team sync, debrief..." value={title} onChange={(e) => setTitle(e.target.value)} />
      </div>
      <div className="flex flex-col gap-1">
          ...
          <div className="flex flex-wrap gap-1 mb-1">
            {guestEmails.map((email) => (
              <span key={email} className="inline-flex items-center gap-1 rounded-full bg-muted px-2 py-0.5 text-caption">
                {email}<button type="button" onClick={() => setGuestEmails((prev) => prev.filter((e) => e !== email))} className="text-muted-foreground hover:text-foreground"><X className="h-3 w-3" /></button>
              </span>
            ))}
          </div>
        )}
