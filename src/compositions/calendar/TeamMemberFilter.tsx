...
/** Props for {@link TeamMemberFilter}. */
...
/**
 * Chip-based team member filter for calendar events.
 * Selected members shown as removable chips. Search to add more.
 * Current user defaults on reload (handled by parent).
 */
export function TeamMemberFilter({
  ...
  const selectedMembers = useMemo(
    () => members.filter((m) => selectedIds.has(m._id)),
    [members, selectedIds]
  );
  ...
  const searchResults = useMemo(() => {
    ...
    return members
      .filter((m) => !selectedIds.has(m._id) && m.name.toLowerCase().includes(q))
      .slice(0, 5);
    ...
    <div ref={containerRef} className="flex flex-col gap-2">
      ...
      <div className="relative">
          ...
          <div className="absolute top-full left-0 right-0 z-10 mt-1 rounded-md border bg-popover shadow-md">
            {searchResults.map((member) => (
              ...
              >
                ...
                <span className="ml-auto text-caption text-muted-foreground capitalize">
        ...
        {dropdownOpen && search.trim() && searchResults.length === 0 && (
          <div className="absolute top-full left-0 right-0 z-10 mt-1 rounded-md border bg-popover shadow-md">
            <p className="px-2 py-2 text-caption text-muted-foreground text-center">
              No members found
            </p>
          </div>
        )}
...
/** Removable chip showing a selected member. */
function MemberChip({
  name,
  avatarUrl,
  onRemove,
...
function MemberAvatar({ name, avatarUrl }: { name: string; avatarUrl?: string }) {
  ...
  return (
    <div className="flex h-4 w-4 items-center justify-center rounded-full bg-primary/10 text-signal-2 font-medium">
      {initials}
    </div>
  );
}
