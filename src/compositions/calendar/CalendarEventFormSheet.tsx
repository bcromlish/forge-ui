...
/** Pre-filled data from quick-create popover. */
...
/** Props for {@link CalendarEventFormSheet}. */
...
/**
 * Full meeting creation form in a Sheet panel.
 * Opened via "More options" from the quick-create popover.
 */
        ...
        <div className="flex flex-col gap-4 px-6 pt-4">
                ...
                {members.map((m) => (
                  <button key={m._id} type="button" onClick={() => toggleMember(m._id)}
                    className={`rounded-full px-2 py-0.5 text-caption border transition-colors ${
                      selectedMemberIds.has(m._id) ? "bg-primary/10 border-primary/30 text-primary" : "bg-muted border-transparent text-muted-foreground hover:border-border"
                    }`}>{m.name}</button>
                ))}
              </div>
            </div>
          ...
          <div className="flex flex-col gap-1.5">
              ...
              <div className="flex flex-wrap gap-1">
                {guestEmails.map((email) => (
                  <span key={email} className="inline-flex items-center gap-1 rounded-full bg-muted px-2 py-0.5 text-caption">
                    {email}
                    <button type="button" onClick={() => setGuestEmails((prev) => prev.filter((e) => e !== email))} className="text-muted-foreground hover:text-foreground">
                      <X className="h-3 w-3" />
                    </button>
                  </span>
                ))}
            ...
            <div className="flex gap-1">
              ...
              <Button type="button" variant="ghost" size="icon" onClick={handleAddGuest} disabled={!guestEmail.trim()}>
