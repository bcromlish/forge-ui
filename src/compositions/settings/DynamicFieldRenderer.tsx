...
/** Props for {@link DynamicFieldRenderer}. */
export interface DynamicFieldRendererProps {
  /** Field definitions that describe the form fields to render. */
  definitions: FieldDefinition[];
  /** Current values keyed by fieldName. */
...
/**
 * Renders a list of dynamic form fields based on field definitions.
 * Each field type maps to the appropriate input component.
 */
    ...
    <div className="flex flex-col gap-4">
      {definitions.map((def) => (
        <DynamicField
          ...
          onChange={(value) => onChange(def.fieldName, value)}
        />
      ))}
    </div>
  );
...
// ---------------------------------------------------------------------------
// Internal — single dynamic field
// ---------------------------------------------------------------------------

/** Props for a single dynamic field. */
...
// ---------------------------------------------------------------------------
// Internal — field input by type
// ---------------------------------------------------------------------------

/** Props for the type-specific input. */
...
/** Maps a FieldType to the appropriate input component. */
function FieldInput({ fieldName, fieldType, value, options, onChange }: FieldInputProps) {
  ...
  switch (fieldType) {
    ...
    case "multiSelect":
      return <MultiSelectField value={value} options={options} onChange={onChange} />;
...
// ---------------------------------------------------------------------------
// Internal — multi-select checkbox group
// ---------------------------------------------------------------------------

/** Props for the multi-select checkbox group. */
